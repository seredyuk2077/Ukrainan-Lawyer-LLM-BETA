...
Окремо `UNGROUNDED_MULTI_GOAL_FALLBACK` тепер має явну нормалізацію: legally-specific bundle без indexed grounding переходить у `likely_missing_act`, а той самий слабкий bundle з metadata grounding лишається `weak_evidence`.
