...
- `scripts/lexery-legal-agent/retrieval/act-taxonomy-store.ts` — LLDBI vocabulary cache (TTL + fallback snapshot), including exact structured act identifiers (`2811-20`, `115/2015`, `100-95-п`, `v0003359-26`) as first-class taxonomy signals
  ...
  and soft exact-alias grounding for code abbreviations (`ЦКУ`, `КПК`, `КУпАП`) even when the surrounding query contains another legal cue like `договір` or `позов`; short alias signals no longer borrow an unrelated cue from the rest of the question and therefore keep code grounding stable in prod-style soft queries
...
- `scripts/lexery-legal-agent/retrieval/finalization/` — final selected-acts normalization cluster (`selected-acts-finalizer.ts`, `single-goal-honesty.ts`, `single-goal-final-honesty.ts`, `coverage-gap.ts`) that owns low-confidence narrowing, final `selected_acts` meta recomputation, and `coverage_gap` derivation
...
- Multi-goal late normalization now gets one bounded uncovered-goal recovery pass before low-confidence trim: if a dropped act has real goal coverage plus strong chunk/taxonomy support, runtime may restore that companion act (for example `law + procedure` or three-act contrastive bundles) instead of collapsing early to an incomplete but overconfident pair.
...
- Для жорсткішого soft multi-question audit тримайте окремий `_datasets/soft_legal_prod_queries_cicero_2026_03_29.json`: це jurist-subagent pack із природними запитами на кшталт `склад + хто розслідує`, `строки + форум`, `санкція + хто розглядає`, `обов'язок + межа між admin/criminal`, `виконавче провадження + арешт + зняття обмежень`. Він особливо корисний для ловлі false multi-goal split і overly cautious `low_confidence` tails на продових формулюваннях.
