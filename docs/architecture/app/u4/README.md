...
- `run_final_manual_audit.ts` — batch manual audit для soft production-style юридичних питань. Для warm/lawyer-style smoke runs без прямого `за законом X` використовуйте `_datasets/soft_legal_prod_queries_2026_03_29.json`; цей pack добре висвітлює `law+bylaw`, `code+procedure`, `law+KSU`, `bylaw+bylaw` і honest `likely_missing_act` surface.
