...
export default function ContractGenerator({ onBack }: ContractGeneratorProps) {
  ...
  const handleDownloadWord = async () => {
    ...
    try {
      await generateWordDocument(generatedContract, `${selectedTemplate.name}.docx`, formData);
  ...
  const handleGenerateMultiple = () => {
    ...
    variations.forEach((variation, index) => {
      ...
      setTimeout(() => {
        generateWordDocument(contract, `${selectedTemplate.name}_варіант_${index + 1}.docx`, formData);
      }, index * 500);
    });
  ...
  return (
    <div className="h-screen flex flex-col bg-white overflow-hidden">
      {/* Header */}
      <div className="p-5 border-b border-slate-200/50 bg-gradient-to-r from-white to-slate-50/50">
        <div className="flex items-center justify-between">
          <div className="flex items-center space-x-4">
            <Button
              ...
              variant="ghost"
              size="sm"
              className="text-slate-600 hover:bg-slate-100 border border-slate-200 rounded-xl px-4 py-2"
            >
            ...
            <div>
              <h2 className="text-xl font-bold text-slate-800 flex items-center">
                ...
                Генератор договорів
                <span className="ml-3 px-3 py-1 text-sm font-medium bg-blue-100 text-blue-700 rounded-full">
                  AI
                </span>
              </h2>
              <p className="text-sm text-slate-500 mt-1">Створення професійних правових документів</p>
            </div>
          </div>
      ...
      <div className="flex-1 grid grid-cols-12 gap-6 p-6 bg-slate-50 overflow-y-auto scrollbar-thin scrollbar-thumb-slate-300 scrollbar-track-transparent hover:scrollbar-thumb-slate-400" style={{ scrollBehavior: 'smooth' }}>
        {/* Template Selection */}
        <motion.div 
                  ...
                  {generatedContract && (
                    ...
                    >
                      <Card className="border-green-200 bg-gradient-to-br from-green-50 to-emerald-50">
                        ...
                        <CardContent>
                          <div className="bg-white rounded-lg p-6 border border-green-200 max-h-96 overflow-y-auto scrollbar-thin scrollbar-thumb-slate-300 scrollbar-track-transparent hover:scrollbar-thumb-slate-400" style={{ scrollBehavior: 'smooth' }}>
