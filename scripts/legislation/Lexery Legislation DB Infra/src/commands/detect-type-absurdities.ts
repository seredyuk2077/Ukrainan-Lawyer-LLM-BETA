...
export function detectAbsurdities(params: {
  ...
  // IMPORTANT: avoid false positives for "ксу" inside words like "кодексу"
  // We only treat "КСУ" as an abbreviation when it appears as a standalone token.
  const hasStandaloneKsu = /(^|[^a-zа-яіїєґ])ксу([^a-zа-яіїєґ]|$)/i.test(combined);
  
  // Snippet-first priority helpers (використовуються в правилах A і D)
  const snippetLower = snippet?.toLowerCase() || '';
  const hasUkazInSnippet = snippetLower.includes('указ') && snippetLower.includes('президент');
  const hasUkazInSummary = summary?.toLowerCase().includes('указ') && summary?.toLowerCase().includes('президент');
  
  // A) RNBO decision (КРИТИЧНЕ: має найвищий пріоритет)
  // ВАЖЛИВО: snippet-first priority (якщо snippet каже "Указ", то це presidential_decree, навіть якщо summary каже "РНБО")
  ...
  if (combined.includes('рада національної безпеки') || 
    ...
    // Виняток 1: Якщо snippet каже "Указ Президента" → це presidential_decree (навіть якщо summary каже "РНБО")
    if (hasUkazInSnippet) {
      ...
      // Якщо snippet каже "Указ", але slug = rnbo_decision → помилка (окрема перевірка)
      if (current_slug === 'rnbo_decision') {
        findings.push({
          nreg: '',
          severity: 'CRITICAL',
          reason_code: 'PRESIDENTIAL_DECREE_AS_RNBO',
          current_slug,
          current_ua_label,
          suggested_slug: 'presidential_decree',
          evidence: { title, summary_prefix: summary?.substring(0, 120), snippet200: snippet?.substring(0, 200), typ, organs },
    ...
    // Виняток 2: Якщо snippet відсутній, але summary каже "Указ про введення в дію рішення РНБО" → presidential_decree OK
    else if (hasUkazInSummary && (summary?.toLowerCase().includes('введення в дію') || summary?.toLowerCase().includes('введення в дію рішення'))) {
      // Це указ про введення в дію рішення РНБО → presidential_decree правильний
      if (current_slug !== 'presidential_decree') {
        findings.push({
          nreg: '',
    ...
    // Якщо НЕ указ про введення в дію → має бути rnbo_decision
    else {
  ...
  // D) Президент
  // ВАЖЛИВО: snippet має пріоритет над summary (snippet = перші рядки документа)
  // Використовуємо hasUkazInSnippet та hasUkazInSummary, визначені на початку функції
  const summaryLower = summary?.toLowerCase() || '';
  
  // Перевірка 2: Розпорядження Президента (snippet-first, але тільки якщо snippet явно каже "розпорядження")
  const hasRozporyadzhenniaInSnippet = snippetLower.includes('розпорядження') && snippetLower.includes('президент');
  const hasRozporyadzhenniaInSummary = summaryLower.includes('розпорядження') && summaryLower.includes('президент');
  
  // Якщо snippet каже "Указ" → presidential_decree (навіть якщо summary каже "розпорядження")
  if (hasUkazInSnippet) {
    ...
    // Якщо snippet каже "Указ", але slug = presidential_order → помилка
    if (current_slug === 'presidential_order') {
      findings.push({
        nreg: '',
        severity: 'CRITICAL',
        reason_code: 'PRESIDENTIAL_DECREE_AS_ORDER',
        current_slug,
        current_ua_label,
        suggested_slug: 'presidential_decree',
        evidence: { title, summary_prefix: summary?.substring(0, 120), snippet200: snippet?.substring(0, 200), typ, organs },
  ...
  // Якщо snippet каже "Розпорядження Президента" (і НЕ "Указ") → presidential_order
  else if (hasRozporyadzhenniaInSnippet) {
    if (current_slug === 'regulation' || current_slug === 'law' || current_slug === 'code' || current_slug === 'presidential_decree') {
      findings.push({
        nreg: '',
        severity: 'CRITICAL',
        reason_code: 'PRESIDENTIAL_ORDER_AS_REGULATION',
        current_slug,
        current_ua_label,
        suggested_slug: 'presidential_order',
        evidence: { title, summary_prefix: summary?.substring(0, 120), snippet200: snippet?.substring(0, 200), typ, organs },
      });
    }
  }
  // Якщо snippet немає, але summary каже "Указ" → presidential_decree
  else if (hasUkazInSummary && !snippet) {
    if (current_slug === 'regulation' || current_slug === 'law' || current_slug === 'code' || current_slug === 'presidential_order') {
      findings.push({
        nreg: '',
        severity: 'CRITICAL',
        reason_code: 'PRESIDENT_AS_REGULATION',
  ...
  // Якщо snippet немає, але summary каже "Розпорядження Президента" (і НЕ "Указ") → presidential_order
  else if (hasRozporyadzhenniaInSummary && !snippet && !hasUkazInSummary) {
    if (current_slug === 'regulation' || current_slug === 'law' || current_slug === 'code' || current_slug === 'presidential_decree') {
      findings.push({
        nreg: '',
        severity: 'CRITICAL',
  ...
  if (combined.includes('окрема думка') || 
    ...
    if (current_slug === 'ccu_opinion' && 
      ...
      if (combined.includes('рішення') && hasStandaloneKsu && !combined.includes('окрема думка')) {
        findings.push({
          nreg: '',
          severity: 'WARN',
  ...
  // Правило 3: document_number має суфікс -РП (Розпорядження Президента, КРИТИЧНЕ)
  if (params.document_number) {
    const normalizedDocNum = normalizeDocNumberForRules(params.document_number);
    // Перевірка суфіксу -РП (з word boundary для безпеки)
    if (normalizedDocNum.match(/-РП\b$/i) || normalizedDocNum.endsWith('-РП')) {
      if (current_slug !== 'presidential_order') {
        // Додаткова перевірка: чи title/snippet/summary підтверджує
        const hasRozporyadzhennia = normalizedSnippet.includes('РОЗПОРЯДЖЕННЯ') || 
                                     normalizedTitle.includes('РОЗПОРЯДЖЕННЯ') ||
                                     (summary && normalizePrefix(summary).includes('РОЗПОРЯДЖЕННЯ'));
        const hasPresident = normalizedSnippet.includes('ПРЕЗИДЕНТ') || 
                            normalizedSnippet.includes('ПРЕЗИДЕНТА') ||
                            normalizedTitle.includes('ПРЕЗИДЕНТ') ||
                            normalizedTitle.includes('ПРЕЗИДЕНТА');
        
        // Якщо є хоча б один сигнал → CRITICAL
        if (hasRozporyadzhennia || hasPresident) {
          findings.push({
            nreg: '',
            severity: 'CRITICAL',
            reason_code: 'PRESIDENTIAL_ORDER_NREG_SUFFIX',
            current_slug,
            current_ua_label,
            suggested_slug: 'presidential_order',
            evidence: { title, summary_prefix: summary?.substring(0, 120), snippet200: snippet?.substring(0, 200), typ, organs },
          ...
          // Навіть без snippet, -РП suffix достатній для CRITICAL
          findings.push({
            nreg: '',
            severity: 'CRITICAL',
            reason_code: 'PRESIDENTIAL_ORDER_NREG_SUFFIX',
  ...
  // Правило 4: document_number має суфікс -РГ (Розпорядження Голови ВРУ, КРИТИЧНЕ)
  if (params.document_number) {
    const normalizedDocNum = normalizeDocNumberForRules(params.document_number);
  ...
  if (combined.includes('рішення')) {
    if (combined.includes('рнбо') && current_slug !== 'rnbo_decision' && current_slug !== 'presidential_decree') {
    ...
    } else if (combined.includes('конституційного суду') || hasStandaloneKsu) {
      ...
      if (combined.includes('окрема думка') && current_slug === 'ccu_opinion') {
        ...
        // Тільки якщо явно "Рішення КСУ" без "окрема думка"
        if (combined.includes('рішення') && hasStandaloneKsu && !combined.includes('окрема думка') && !combined.includes('щодо')) {
          findings.push({
            nreg: '',
            severity: 'WARN',
            reason_code: 'CCU_DECISION_NOT_CCU',
