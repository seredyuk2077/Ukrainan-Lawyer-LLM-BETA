...
export async function auditParserIntegrityV2(options?: {
  ...
  const markdownFile = outputFile?.replace('.json', '.md') || resolve(process.env.LEXERY_LEGISLATION_WORKSPACE_ROOT || process.cwd(), 'runs', 'audit', 'PARSER_INTEGRITY_V2_REPORT.md');
  const fs = await import('fs/promises');
