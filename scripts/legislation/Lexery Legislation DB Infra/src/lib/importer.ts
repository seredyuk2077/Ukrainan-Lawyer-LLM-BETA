...
export async function importOne(opts: ImportOptions): Promise<ImportResult> {
  ...
  const canonical: CanonicalDocument = await buildCanonical({
    ...
    previousHash,
    radaClient: rada, // Передаємо RadaClient для resolver
  });
  ...
  try {
    ...
    const validation = await validateDocumentTypeConsistency(
      (canonical.metadata.document_type_slug || 'unknown') as any,
    ...
    // PHASE 16: sync_health (буде оновлено в verify)
    
    // PHASE 5: Фінальна валідація після AI summary generation
    const finalValidation = await validateDocumentTypeConsistency(
      (canonical.metadata.document_type_slug || 'unknown') as any,
      canonical.metadata.title,
    ...
    const docUpsert = {
      ...
      // legal_status column was removed; sync_health/sync_issue залишаємо як soft hints
      ...
      // Validity fields (PROD PIPELINE: IDEAL DATA CONTRACT — НЕ NULL для текстових)
      validity_status: canonical.metadata.validity_status || 'unknown',
      status_note: canonical.metadata.status_note || 'no_evidence', // НЕ NULL
      source_status_text: canonical.metadata.source_status_text || 'N/A', // НЕ NULL
      source_status_location: canonical.metadata.source_status_location || 'fallback.no_evidence', // НЕ NULL
    };
