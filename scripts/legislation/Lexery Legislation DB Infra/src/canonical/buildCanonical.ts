...
export interface CanonicalMetadata {
  ...
  r2_key?: string;
  // Validity fields (REFACTOR)
  validity_status?: 'ACTIVE' | 'REPEALED' | 'NOT_IN_FORCE' | 'PARTIALLY_IN_FORCE' | 'UNKNOWN';
...
export interface BuildCanonicalOptions {
  ...
  radaClient?: any; // RadaClient (optional, для resolver)
}
...
export async function buildCanonical(options: BuildCanonicalOptions): Promise<CanonicalDocument> {
  const { jsonPath, txtPath, previousHash, radaClient } = options;
  ...
  // PRIORITY LADDER: raw_txt має найвищий пріоритет для prefix-sniff (для декретів, НКРЕКП, KIND-FIRST)
  // ВАЖЛИВО: для декретів та НКРЕКП потрібно брати raw_txt, бо chunks можуть не містити заголовок
  ...
  // Витягуємо чинність (PROD PIPELINE: через resolver якщо доступний)
  const canonicalTopBlock = txtContent ? txtContent.substring(0, 2500) : null;
  let validity: any;
  
  if (radaClient && nreg) {
    // Використовуємо resolver (authoritative source)
    const { extractValidityAsync } = await import('./extractValidity.js');
    validity = await extractValidityAsync(
      nreg,
      radaClient,
      jsonData,
      txtContent || null,
    ...
    // Fallback на синхронну версію (legacy)
    const { extractValidity } = await import('./extractValidity.js');
    validity = extractValidity(jsonData, txtContent, canonicalTopBlock, documentTypeSlug, nazva);
  }
  ...
  const canonical: CanonicalDocument = {
    ...
    metadata: {
      ...
      r2_key: r2Key,
      // Validity fields (PROD PIPELINE: НЕ NULL validity_status)
      validity_status: validity.validity_status || 'unknown', // DEFAULT 'unknown' якщо не встановлено
