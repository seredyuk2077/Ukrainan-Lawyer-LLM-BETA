...
export async function searchDocuments(opts: SearchOptions): Promise<void> {
  ...
  // Small quality boost for explicit article queries:
  // If user explicitly writes "стаття 115" / "ст. 257" — we retrieve a wider candidate set
  // and then reorder results by exact article_number match (no Qdrant payload filtering/index required).
  const mArticle = opts.query.match(/(?:\bст\.?\b|\bстатт[яі]\b)\s*(\d{1,4})/iu);
  const explicitArticle = mArticle ? String(Number(mArticle[1])) : null;

  const candidateLimit = explicitArticle ? Math.max(topk, 50) : topk;
  const general = await qdrant.search(QDRANT_COLLECTION_CHUNKS, {
  ...
  let searchResults = general;
  if (explicitArticle) {
    const target = explicitArticle;
    searchResults = [...general].sort((a: any, b: any) => {
      const pa = a.payload || {};
      const pb = b.payload || {};
      const aHit = String(pa.article_number ?? '') === target ? 1 : 0;
      const bHit = String(pb.article_number ?? '') === target ? 1 : 0;
      if (aHit !== bHit) return bHit - aHit; // bring exact matches first
