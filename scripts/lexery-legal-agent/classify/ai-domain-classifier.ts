...
const SITUATION_GUIDANCE =
  'Визначай галузь за типом ПИТАННЯ (про що питають): питання про кримінальну відповідальність/кваліфікацію/покарання/статті ККУ → criminal; питання про відшкодування шкоди, моральну шкоду, позов, стягнення → civil навіть якщо в описі є заподіяння шкоди; питання про штраф, позбавлення права керування, хто розглядає справу про адмінправопорушення, керування у стані сп’яніння, типові КУпАП-сюжети → administrative_offenses; оскарження рішень органів влади, адмінсуд, доступ до публічної інформації → administrative; юрособи, банкрутство → corporate. Один факт може бути кримінальним або цивільним питанням — дивись на формулювання питання.';
