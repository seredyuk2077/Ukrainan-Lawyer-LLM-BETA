...
| `u4/verify_lldbi_absent_present.ts` | — | Batch absent→present LLDBI verifier: перевіряє honest `likely_missing_act`/`weak_evidence` path для explicit absent-act queries, далі імпортує акти через LLDBI admin CLI, запускає `verify` + `audit-qdrant-payload`, перезапускає retrieval на тих самих queries і пише JSON report. Історичні `seed/fresh/diverse/systemic` packs тепер часто вже present-only; для живого missing-act benchmark використовуйте `_datasets/lldbi_absent_present_live_absent_cases.json`, `_datasets/lldbi_absent_present_turbo_cases.json` і rotating live shards. Presence detection тут retrieval-surface-first: verifier нормалізує `rada_nreg` variants і вважає case stale, якщо act уже видимий через Supabase або Qdrant, навіть коли старий corpus зберіг case-variant payload. Verifier також явно показує `stale_pre_dataset=true`, якщо pre-phase вичерпалася через already-present corpus, і підтримує successor-aware post checks через `acceptable_post_selected_acts` для soft current-law queries. |
