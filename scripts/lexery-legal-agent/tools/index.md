...
| `u4/run_final_manual_audit.ts` | — | Manual audit batch для production-style legal queries. Для soft/warm lawyer-style smoke прогонів використовуйте `--dataset=scripts/lexery-legal-agent/tools/_datasets/soft_legal_prod_queries_2026_03_29.json`, для ширшого lawyer-style prod audit — `--dataset=scripts/lexery-legal-agent/tools/_datasets/soft_legal_prod_queries_subagent_2026_03_29.json`, а для особливо жорсткого soft multi-question regression pack — `--dataset=scripts/lexery-legal-agent/tools/_datasets/soft_legal_prod_queries_cicero_2026_03_29.json`; ці набори перевіряють `selected_acts`, `low_confidence`, latency і missing-act honesty без прямого `за законом X`. |
| `u4/verify_lldbi_absent_present.ts` | — | Batch absent→present LLDBI verifier: перевіряє honest `likely_missing_act`/`weak_evidence` path для absent-act queries, далі імпортує акти через LLDBI admin CLI, запускає `verify` + `audit-qdrant-payload`, перезапускає retrieval на тих самих queries і пише JSON report. Історичні `seed/fresh/diverse/systemic` packs тепер часто вже present-only; для живого missing-act benchmark використовуйте `_datasets/lldbi_absent_present_live_absent_cases.json`, `_datasets/lldbi_absent_present_live_absent_shard_b_cases.json`, `_datasets/lldbi_absent_present_soft_live_2026_03_28_cases.json` і `_datasets/lldbi_absent_present_diverse_live_2026_03_28_cases.json`. Presence detection тут retrieval-surface-first: verifier нормалізує `rada_nreg` variants і вважає case stale, якщо act уже видимий через Supabase або Qdrant, навіть коли старий corpus зберіг case-variant payload. Verifier також явно показує `stale_pre_dataset=true`, якщо pre-phase вичерпалася через already-present corpus, підтримує successor-aware post checks через `acceptable_post_selected_acts` для soft current-law queries і вимагає, щоб lead `selected_act` теж входив у очікуваний набір. |
| `u4/audit_lldbi_act_coverage.ts` | `brain:audit:lldbi-act-coverage` | Broad LLDBI act-coverage audit по багатьох indexed актах; `--probe-mode=generalized` запускає кілька query families на кожен акт (`nreg`, `alias`, `anchored title`, `title fragment`, а за наявності cue-патернів — і `document+№number` phrasing), broad runs forced to LLDBI-only (`DOCLIST_ENABLED=false`), honesty leaks count as failures, а `--report-path` + strict `--resume` validation + atomic checkpoints дозволяють робити shard-based corpus sweeps і кластеризувати generalized failures. `best`-mode тепер truthfulness-first: слабкі boilerplate alias-и відсікаються, а для amendment/repeal-like acts harness падає назад до власного `nreg`, якщо alias не несе надійної identity самого indexed акту |
