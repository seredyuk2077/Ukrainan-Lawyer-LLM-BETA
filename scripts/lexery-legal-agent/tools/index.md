...
| `u4/run_final_manual_audit.ts` | — | Manual audit batch для production-style legal queries. Для soft/warm lawyer-style smoke прогонів використовуйте `--dataset=scripts/lexery-legal-agent/tools/_datasets/soft_legal_prod_queries_2026_03_29.json`, а для ширшого lawyer-style prod audit — `--dataset=scripts/lexery-legal-agent/tools/_datasets/soft_legal_prod_queries_subagent_2026_03_29.json`; обидва набори перевіряють `selected_acts`, `low_confidence`, latency і missing-act honesty без прямого `за законом X`. |
